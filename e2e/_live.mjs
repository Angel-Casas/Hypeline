...
const b = await chromium.launch({ executablePath: process.env.CHROMIUM, args: ['--no-proxy-server'] });
...
await ctx.addInitScript(() => {
  ...
  window.__notes = [];
  class N { static permission = 'granted'; static requestPermission = async () => 'granted'; constructor(t, o) { window.__notes.push(t + ' | ' + o.body); } close() {} }
...
await p.waitForFunction(() => /chat connected|is offline|Could not/.test(document.body.innerText), null, { timeout: 30000 });
await p.evaluate(() => { window.__hidden = true; });
const N = Number(process.env.N ?? 20);
for (let i = 0; i < N; i++) {
  await p.waitForTimeout(15000);
  const line = await p.locator('.font-mono.text-\\[11px\\]').first().innerText().catch(() => '');
  const feed = await p.locator('ol li').count();
  console.log(`${(i + 1) * 15}s:`, line.replace(/\s+/g, ' '), '· feed', feed, '· title', await p.title());
}
