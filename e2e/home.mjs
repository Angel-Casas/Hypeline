...
await p.route('https://api.twitch.tv/helix/**', (route) => {
  ...
  if (u.pathname.endsWith('/users'))
    return route.fulfill({
      json: { data: [{ id: '1', profile_image_url: 'https://static-cdn.jtvnw.net/x.png' }] },
    });
        ...
        data: [
          {
            ...
            thumbnail_url:
              'https://static-cdn.jtvnw.net/previews-ttv/live_user_quiet-{width}x{height}.jpg',
          },
        ],
      },
    });
        ...
        data: [
          {
            ...
            view_count: 10,
            thumbnail_url:
              'https://static-cdn.jtvnw.net/cf_vods/x/thumb/thumb-%{width}x%{height}.jpg',
          },
        ],
      },
    });
...
await p.route('https://gql.twitch.tv/gql', async (route) => {
  ...
  return route.fulfill({
    json: [{ errors: [{ message: 'x' }], data: { video: { comments: null } } }],
  });
});
...
await p.goto(
  BASE + '/dashboard#access_token=tok123&scope=user%3Aread%3Afollows&state=s1&token_type=bearer',
);
await p.waitForFunction(() => /Your Twitch · angel/i.test(document.body.innerText), null, {
  timeout: 15000,
});
await p.waitForFunction(() => /Latest VODs · 1/i.test(document.body.innerText), null, {
  timeout: 15000,
});
...
console.log(
  'cards:',
  cards.map((c) => c.replace(/\s+/g, ' ')),
);
if (!cards.some((c) => /LIVE/.test(c)) || !cards.some((c) => /Fixture VOD/.test(c)))
  throw new Error('home cards missing');
...
await p.waitForFunction(
  () => /Fixture VOD/.test(document.querySelector('h1')?.textContent ?? ''),
  null,
  { timeout: 30000 },
);
