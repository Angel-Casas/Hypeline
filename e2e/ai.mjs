...
await p.route('https://nano-gpt.com/api/**', async (route) => {
  ...
  if (u.includes('/v1/audio/transcriptions')) {
    ...
    seen.stt = {
      ...
      isMp3: body.includes('filename="a.mp3"'),
      hasModel: body.includes('Whisper-Large-V3'),
    };
    return route.fulfill({
      json: { text: 'grandma what are you doing', language: 'en', duration: 15 },
    });
  }
  if (u.includes('/v1/chat/completions')) {
    ...
    seen.chat = {
      model: j.model,
      hasSchema: j.response_format?.type === 'json_schema',
      promptHasTranscript: j.messages[1].content.includes('grandma what are you doing'),
    };
    return route.fulfill({
      json: {
        choices: [
          {
            message: {
              content: JSON.stringify({
                title: 'Grandma pulls up',
                hook: 'Nobody expected this',
                why: 'Chat lost it',
                suggestedInOffset: 2,
                suggestedOutOffset: 14,
                clipWorthiness: 5,
              }),
            },
          },
...
await p.waitForFunction(
  () => document.body.innerText.includes('grandma what are you doing'),
  null,
  { timeout: 120000 },
);
...
if (
  !seen.stt?.hasModel ||
  !seen.stt.isMp3 ||
  seen.stt.bodyBytes < 40_000 ||
  seen.stt.bodyBytes > 120_000
)
  throw new Error('unexpected transcription upload: ' + JSON.stringify(seen.stt));
console.log(
  `mp3 upload ≈ ${Math.round(seen.stt.bodyBytes / 1024)} KB (expected ≈ 60 KB for 15 s @ 32 kbps)`,
);

const eBtn = p.getByRole('button', { name: /^Explain/ });
console.log('explain label:', (await eBtn.innerText()).trim());
await eBtn.click();
await p.waitForFunction(() => document.body.innerText.includes('Grandma pulls up'), null, {
  timeout: 30000,
});
console.log('chat request:', JSON.stringify(seen.chat));
if (!seen.chat?.hasSchema || !seen.chat.promptHasTranscript)
  throw new Error('explain prompt missing schema or transcript');
...
await p.waitForFunction((n) => document.querySelectorAll('video').length > n, vidsBefore, {
  timeout: 300000,
});
...
execFileSync('ffmpeg', [
  ...
  '1',
  '-vf',
  'crop=iw:ih*0.25:0:ih*0.7,format=gray',
  frame,
]);
const stats = execFileSync('ffprobe', [
  '-v',
  'error',
  '-f',
  'lavfi',
  '-i',
  `movie=${frame},signalstats`,
  '-show_entries',
  'frame=pkt_pts_time:frame_tags=lavfi.signalstats.YMAX,lavfi.signalstats.YAVG',
  '-of',
  'json',
]).toString();
const tags = JSON.parse(stats).frames[0].tags;
console.log(
  'captioned 9:16 export:',
  meta.replace(/\s+/g, ' ').slice(0, 90),
  '· bottom band YMAX',
  tags['lavfi.signalstats.YMAX'],
  'YAVG',
  tags['lavfi.signalstats.YAVG'],
);
if (Number(tags['lavfi.signalstats.YMAX']) < 230)
  throw new Error('no bright caption pixels found in the bottom band');
